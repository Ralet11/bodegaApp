...
import {
  ...
  Dimensions, ScrollView, Animated, Linking, Platform
} from 'react-native';
...
export default function MapViewComponent() {
  ...
  // ------------------------------------------------------------------------------------
  //   A N I M A T I O N   when card opens/closes
  // ------------------------------------------------------------------------------------
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const s of shops) {
        try {
          const { data } = await Axios.get(`https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=${encodeURIComponent(address)}&destinations=${encodeURIComponent(s.address)}&key=${GOOGLE_API_KEY}`);
  ...
  const openAddressMaps = addr => {
    ...
    const url = Platform.OS==='ios' ? `http://maps.apple.com/?q=${enc}` : `https://www.google.com/maps/search/?api=1&query=${enc}`;
    Linking.openURL(url);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <BlurView intensity={100} style={[styles.headerContainer,{ marginTop:insets.top }]}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tagsScrollView}>
          {allTags.map(t=>(
            ...
            >
              <Text style={[styles.tagText, selectedTag?.id===t.id && styles.selectedTagText]}>{t.name}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </BlurView>
      ...
      {loading ? (
        <View style={styles.loaderContainer}>
          <LottieView source={{ uri:'https://assets5.lottiefiles.com/packages/lf20_X7WDCg.json' }} autoPlay loop style={styles.lottieAnimation}/>
        </View>
      ) : permissionDenied ? (
        <View style={styles.permissionDeniedContainer}>
          <LottieView source={{ uri:'https://assets5.lottiefiles.com/packages/lf20_sB03tR.json' }} autoPlay loop style={styles.lottieAnimation}/>
          <Text style={styles.permissionDeniedText}>Location access is required.</Text>
        </View>
      ) : region && (
        <MapView style={styles.map} region={region}>
          {marker && <Marker coordinate={marker}><FontAwesome5 name="map-pin" size={24} color="#4A90E2"/></Marker>}
          {shops.map((s,i)=>(
            <Marker key={s.id} coordinate={{ latitude:s.lat, longitude:s.lng }} onPress={()=>setSelectedIdx(i)} calloutEnabled={false}>
              <View style={styles.shopMarkerContainer}>
                {s.logo
                  ? <Image source={{ uri:s.logo }} style={styles.shopMarkerImage}/>
                  : <View style={styles.defaultMarker}><FontAwesome5 name="store" size={18} color="#fff"/></View>}
              </View>
            </Marker>
          ))}
        </MapView>
      ...
      {selectedIdx!==null && (
        <Animated.View style={[
          ...
          <FlatList
            ...
            onScrollToIndexFailed={i=>setTimeout(()=>flatRef.current?.scrollToIndex({ index:i.index, animated:true }),500)}
            renderItem={({ item })=>(
              <View style={styles.pageContainer}>
                <View style={styles.modernCardContainer}>
                  <View style={styles.modernImageWrapper}>
                    <Image source={{ uri:item.placeImage }} style={StyleSheet.absoluteFill} resizeMode="cover"/>
                    <View style={styles.imageOverlay}/>
                    <TouchableOpacity style={styles.modernCloseButton} onPress={()=>setSelectedIdx(null)}>
                  ...
                  <View style={styles.modernCardContent}>
                    <View style={styles.modernTitleRow}>
                      <Text style={styles.modernCardTitle} numberOfLines={1}>{item.name}</Text>
                      {!!item.rating && (
                    ...
                    <TouchableOpacity style={styles.modernAddressRow} onPress={()=>openAddressMaps(item.address)}>
                      <FontAwesome5 name="map-marker-alt" size={12} color="#FF9900"/>
                      <Text style={styles.modernAddressText} numberOfLines={2}>{item.address}</Text>
                    </TouchableOpacity>
                    <View style={styles.modernBottomRow}>
                      ...
                      <TouchableOpacity style={styles.modernViewButton} onPress={()=>navigateToShop(item)}>
